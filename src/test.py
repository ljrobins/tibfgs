...
def test_dcsearch():
    ...
    alpha1, phi0, derphi0, maxiter = 0.0011208198582888297, 3.991124871148457, -15.99522009389491, 100
    ...
    def run_ti_dcsrch() -> ti.types.vector(4, ti.f32):
        ...
        task: ti.u8 = 0
        i: ti.i32 = 1
        x = ti.Vector(DCSRCH_ti(VTYPE(xk), VTYPE(pk), ftol, gtol, xtol, stpmin, stpmax, i=i).call(
            alpha1, phi0=phi0, derphi0=derphi0, maxiter=maxiter
        ))
        return x
...
def test_wolfe1():
    ...
    def call_ti_line_search() -> ti.types.vector(n=4, dtype=ti.f32):
        ...
        x = ti_line_search_wolfe1(i=0, xk=xk, pk=pk, gfk=gfk, old_fval=old_fval, old_old_fval=old_old_fval, 
                            amin=1e-10, amax=1e10, c1=1e-4, c2=0.9, xtol=1e-7)
